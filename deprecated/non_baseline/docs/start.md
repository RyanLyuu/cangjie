...
执行类型解析（只物化 Java -> Cangjie 类型契约）：

```bash
bash scripts/java/resolve_types.sh <project> <model_name> <temperature> _evosuite_cleaned_base
```
...
该命令启动骨架阶段 Codex agent。agent 只能通过本地 MCP 工具
`generate_cangjie_skeleton` 生成骨架；工具读取已物化的类型契约、生成两个
TODO skeleton tree，并执行 `cjpm build`。工具回执保存在
`data/java/skeleton_generation_runs/`。
...
```bash
bash scripts/java/resolve_types.sh <project> <model_name> <temperature> <suffix>
```
...
- **Python 脚本：** `src/java/type_resolution/cli.py`（`--types-only`）
...
- **参数说明：**
  ...
  - `<suffix>` - 后缀
  - `<temperature>` - 采样温度
- **MCP 工具：** `src/java/translation/skeleton_mcp_server.py`
- **生成服务：** `src/java/translation/skeleton_service.py`
...
| 类型解析    | `src/java/type_resolution/cli.py`                              | 物化类型契约和类型运行产物                    |
| 骨架 MCP    | `src/java/translation/skeleton_mcp_server.py`                  | 暴露骨架生成工具给 skeleton agent             |
| 骨架服务    | `src/java/translation/skeleton_service.py`                     | 生成 Cangjie skeleton、placeholder 并编译      |
...
│   ├── type_resolution/
│   │   ├── cli.py                 # 类型解析 CLI
│   │   └── agent.py               # Codex runner
│   ├── translation/
│   │   ├── create_skeleton.py    # 骨架渲染实现（由 MCP 服务调用）
│   │   ├── skeleton_mcp_server.py # 骨架 MCP stdio server
│   │   ├── skeleton_service.py   # 骨架生成服务
│   │   └── skeleton_stage.py      # skeleton agent 编排
